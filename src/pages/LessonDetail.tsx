...
import { motion } from 'framer-motion';  // Mantém animações originais
...
const LessonDetail = () => {
  ...
  const toggleFavorite = async () => {
    if (!licao || !userData || isUpdatingFavorite) return;
      ...
      {loading ? (
        ...
        <div className="min-h-screen flex items-center justify-center">
          <Card className="p-8 text-center max-w-md">
            ...
            <p className="mb-6">Assine para acessar esta lição.</p>
            <div className="space-y-3">
              <Button onClick={() => navigate('/upgrade')} className="w-full">Assinar Premium</Button>
              <Button variant="outline" onClick={() => navigate(-1)} className="w-full">Voltar</Button>
            </div>
          </Card>
        </div>
      ) : (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 px-4 sm:px-6 lg:px-8 relative">
          ...
          <div id="pdf-content">
            <Suspense fallback={<div className="text-center py-10 text-purple-500">Carregando...</div>}>
              <motion.div
              ...
              >
                ...
                <div>
                  <h1 className="text-3xl font-bold text-purple-600 flex items-center gap-2">
                    {licao.titulo}
                    {licao.isPremium && (
                      <Crown className="w-6 h-6 text-yellow-500 drop-shadow-md animate-pulse" />
                    )}
                  </h1>
                ...
                {/* Seções com cores, ícones e efeitos preservados */}
                {licao.historia && (
                ...
                {/* Mantendo todas as demais seções exatamente como no original */}
                {/* Aplicação */}
                <Card className="mb-6 py-6 px-3 sm:px-4">
                  <motion.section
                    initial={{ opacity: 0, y: 20 }}
                ...
                {/* Dinâmica, Atividade, Oração - iguais ao original */}
                {licao.dinamica && (
                  <Card className="mb-6 py-6 px-3 sm:px-4">
                    <motion.section
                      initial={{ opacity: 0, y: 20 }}
