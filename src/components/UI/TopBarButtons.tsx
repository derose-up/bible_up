...
// Importação direta (sem lazy)
import { PDFDownloadLink } from '@react-pdf/renderer';
import PdfLicao from '../../components/pdf/PdfLicao';
...
const TopBarButtons: React.FC<TopBarButtonsProps> = ({
  ...
  const btnBaseClasses =
    'p-2 rounded-md cursor-pointer flex items-center justify-center transition-colors';
        ...
        >
          {({ loading }) => (
            <button
              ...
              className={`${btnBaseClasses} text-white border border-white hover:bg-purple-700`}
            >
              {loading ? <LoadingSpinner size="sm" /> : <Download size={24} />}
            </button>
          )}
        </PDFDownloadLink>
      </motion.div>
